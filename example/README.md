...
```bash
example$ sam local invoke ScheduledEventLogger --event events/event-cloudwatch-event.json
```
...
```bash
example$ sam logs -n ScheduledEventLogger --stack-name sam-app --tail
```
...
```bash
example$ dotnet test test/example.Tests/example.Tests.csproj 
```
